...
def get_h5_fname(meshdir, h5_fname=None):
    if h5_fname is not None:
        ...
        h5_files = [file for file in h5_files if not any(substr in file.name for substr in ["fiber", "ffun", "cfun", "laplace", "microstructure"])]
...
geometry = pulse.HeartGeometry(mesh=geo.mesh, markers=geo.markers, microstructure=microstructure, marker_functions=marker_functions)
...
unloader = pulse.FixedPointUnloader(problem=problem, pressure=0.24)
...
fig.add_plot(plot(unloaded_geometry.mesh, opacity=0.5, color="grey", show=False))
fig.show()
# %%
